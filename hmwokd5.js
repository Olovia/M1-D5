...
*/
me.skills = ["Javascript", "Python", "HTML5", "CSS"]
console.log(me)
/* Ex.G 
...
function randomDice(dice) {
    return Math.round(Math.random() * dice + 1);
}

console.log(randomBetween(6))
...
*/
function deleteOne(aString, aBoolean) {
    if (aBoolean === true) {
...
console.log(deleteOne("Love you", true))
/* Ex.5
   Write the function onlyLetters that receives a string, removes all the numbers and returns it.
   Ex.: onlyLetters("I love 123 whatever")  => returns "I love whatever"
*/
/* function onlyLetters(stringWithNu) {
    let stringistr = stringWithNu.split("").map(String)
    console.log(stringistr)
    let haha = stringWithNu.split("").map(Number)
    console.log(haha)
    ...
    /*  console.log(haha)
    for (let i = 0; i < haha.length; i++) {
        if (typeof haha[i] === typeof stringWithNu) {
            let ahhh = haha.splice(i, 1)
            console.log(ahhh)
        }
    } */ 

function onlyLetters(stringWithNu) {
    return (stringWithNu.replace(/\d/g, ""))
}
console.log(onlyLetters("I love 123 you"))
/* Ex.6 
   Write the function isThisAnEmail that receives a string and returns true if the string is a valid email.
*/
function isThisAnEmail(adress) {
    if (adress.includes("@",".com")) {
        console.log(true)
    } else {
        console.log("nice try")
    }
}
isThisAnEmail("olovia@gmail.com")
/* Ex.7
...
function rollTheDices(dices) {
    let a = Math.round(Math.random() * dices + 1)
    let b = Math.round(Math.random() * dices + 1)
    let c = Math.round(Math.random() * dices + 1)
    return rollIt = {
        sum: a + b + c ,
        values: [a, b, c]
    }
